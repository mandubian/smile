 ...
 * @author Haifeng Li
 */
public class QuantileRegressionTree implements QuantileRegression<double[]>, Serializable {
    ...
    public static class Trainer extends RegressionTrainer<double[]> {
        ...
        @Override
        public QuantileRegressionTree train(double[][] x, double[] y) {
            return new QuantileRegressionTree(attributes, x, y, maxNodes, nodeSize);
        }
        
        public QuantileRegressionTree train(int[][] x, double[] y) {
            if (numFeatures <= 0) {
                return new QuantileRegressionTree(Math.max(x) + 1, x, y, maxNodes, nodeSize);
            } else {
                return new QuantileRegressionTree(numFeatures, x, y, maxNodes, nodeSize);
            }
        }
    }
    ...
    class Node implements Serializable {
        ...
        double output = 0.0;
        /**
         * values from training set finally associated to this node while training
         * This is filled only in leaves of tree.
         */
        SparseMatrix leafValues;
        /**
        ...
        /**
         * Evaluate the regression tree over an instance and return prediction distribution.
         */
        public SparseMatrix predictDistribution(double[] x) {
            if (trueChild == null && falseChild == null) {
                return leafValues;
            } else {
                if (attributes[splitFeature].getType() == Attribute.Type.NOMINAL) {
                    if (Math.equals(x[splitFeature], splitValue)) {
                        return trueChild.predictDistribution(x);
                    } else {
                        return falseChild.predictDistribution(x);
                    }
                } else if (attributes[splitFeature].getType() == Attribute.Type.NUMERIC) {
                    if (x[splitFeature] <= splitValue) {
                        return trueChild.predictDistribution(x);
         ...
         * Evaluate the regression tree over an instance and return prediction distribution.
         */
        public SparseMatrix predictDistribution(int[] x) {
            if (trueChild == null && falseChild == null) {
                return leafValues;
            } else if (x[splitFeature] == (int) splitValue) {
                return trueChild.predictDistribution(x);
        ...
        /** Build count/values distribution for a leaf node using trueSamples & falseSamples sparse vectors
          * All 3 arrays should have same number of rows and trueSamples & falseSamples can be null
          */
        public SparseMatrix buildPredictionDistribution(int[] trueSamples, int[] falseSamples, double[] values) {
            int nrows = values.length;
            int ncols = 2;

            int n = 0; // number of non-zero elements
            // samples & trueSamples contain our count now & should have same size
            for (int i = 0; i < nrows; i++) {
                // samples has been rewritten 
                if (trueSamples != null && trueSamples[i] > 0) {
                    n++;
                }
                if (falseSamples != null && falseSamples[i] > 0) {
                    n++;
                }
            }

            // multiply by ncols for total non-zeros elements (samples & values)
            int nb = n * ncols;
            double[] vals = new double[nb];
            int[] rowIndex = new int[nb];
            int[] colIndex = new int[ncols + 1];

            int j = 0;
            // col0 is counts column (stored as double)
            colIndex[0] = 0;
            // col1 is values column
            colIndex[1] = n;
            // stores total number of elements in colIndex[ncols==2]
            colIndex[ncols] = nb;
            ...
            for (int i = 0; i < nrows; i++) {
                if (trueSamples != null && trueSamples[i] > 0) {
                    // set the count
                    rowIndex[j] = i;
                    vals[j] = trueSamples[i];
                    // set the value (in next column)
                    rowIndex[j + n] = i;
                    vals[j + n] = values[i];
                    j++;
                }
                // true & false can't be true together
                else if (falseSamples != null && falseSamples[i] > 0) {
                    // set the count
                    rowIndex[j] = i;
                    vals[j] = falseSamples[i];
                    // set the value (in next column)
                    rowIndex[j + n] = i;
                    vals[j + n] = values[i];
                    j++;
            ...
            //     System.out.printf("(%-7.3f, %-7.3f)", vals[row], vals[row + n]);
            //     System.out.printf("\n");
            // }

            // System.out.println("rowIndex");
            // for (int row = 0; row < rowIndex.length; row ++){
            //     System.out.printf("%d", rowIndex[row]);
            //     System.out.printf("\n");
            // }
            // System.out.println("colIndex");
            // for (int row = 0; row < colIndex.length; row ++){
            //     System.out.printf("%d", colIndex[row]);
            //     System.out.printf("\n");
            // }
            // System.out.println("END");
            leafValues = new SparseMatrix(nrows, ncols, vals, rowIndex, colIndex);
    ...
    class TrainNode implements Comparable<TrainNode> {
        ...
        public boolean split(PriorityQueue<TrainNode> nextSplits) {
            ...
            if (tc < nodeSize || fc < nodeSize) {
                ...
                // in a leaf, we build the distribution of values
                node.buildPredictionDistribution(trueSamples, samples, y);
                return false;
            }
            ...
            if (tc > nodeSize && trueChild.findBestSplit()) {
                ...
                // trueChild is a leaf
                // in a leaf, we build the distribution of values
                node.trueChild.buildPredictionDistribution(trueSamples, null, y);
            }
            ...
            if (fc > nodeSize && falseChild.findBestSplit()) {
                ...
                // falseChild is a leaf
                // in a leaf, we build the distribution of values
                node.falseChild.buildPredictionDistribution(null, samples, y);
            }
     ...
     * @param maxNodes the maximum number of leaf nodes in the tree.
     */
    public QuantileRegressionTree(double[][] x, double[] y, int maxNodes) {
     ...
     * @param maxNodes the maximum number of leaf nodes in the tree.
     */
    public QuantileRegressionTree(double[][] x, double[] y, int maxNodes, int nodeSize) {
     ...
     * @param maxNodes the maximum number of leaf nodes in the tree.
     */
    public QuantileRegressionTree(Attribute[] attributes, double[][] x, double[] y, int maxNodes) {
     ...
     * @param maxNodes the maximum number of leaf nodes in the tree.
     */
    public QuantileRegressionTree(Attribute[] attributes, double[][] x, double[] y, int maxNodes, int nodeSize) {
     ...
     */
    public QuantileRegressionTree(Attribute[] attributes, double[][] x, double[] y, int maxNodes, int nodeSize, int mtry, int[][] order, int[] samples, NodeOutput output) {
        ...
        TrainNode trainNode = nextSplits.poll();
     ...
     * @param maxNodes the maximum number of leaf nodes in the tree.
     */
    public QuantileRegressionTree(int numFeatures, int[][] x, double[] y, int maxNodes) {
     ...
     * not split, setting nodeSize = 5 generally gives good results.
     */
    public QuantileRegressionTree(int numFeatures, int[][] x, double[] y, int maxNodes, int nodeSize) {
     ...
     * samples[i] should be 0 or 1 to indicate if the instance is used for training.
     */
    public QuantileRegressionTree(int numFeatures, int[][] x, double[] y, int maxNodes, int nodeSize, int[] samples, NodeOutput output) {
        if (x.length != y.length) {
            throw new IllegalArgumentException(String.format("The sizes of X and Y don't match: %d != %d", x.length, y.length));
        }
    ...
    /**
     * Predicts the distribution of values for a dependent variable of an instance.
     * @param x the instance.
     * @return the predicted distribution of values for a dependent variable.
     */
    public SparseMatrix predictDistribution(double[] x) {
        return root.predictDistribution(x);
    }
    ...
    public String dot() {
        ...
        builder.append("digraph QuantileRegressionTree {\n node [shape=box, style=\"filled, rounded\", color=\"black\", fontname=helvetica];\n edge [fontname=helvetica];\n");
